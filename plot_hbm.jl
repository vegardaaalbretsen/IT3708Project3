...
function usage()
    println("Usage: julia --project=. plot_hbm.jl <dataset-key|triangle|triangle-asym> [epsilon] [output-path]")
    println("")
    println("Examples:")
...
saved_path = save_hbm_plot(landscape, output_path; title=title, fitness_label=fitness_label)
